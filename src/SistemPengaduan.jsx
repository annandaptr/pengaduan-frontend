...
const jenisPengaduan = [
  { id: "akta", icon: "📄", title: "Akta Kelahiran", desc: "Perbaikan data pada akta kelahiran", color: "#1a6b4a" },
  { id: "kk", icon: "🏠", title: "Kartu Keluarga", desc: "Perubahan atau perbaikan data KK", color: "#1a4b6b" },
  { id: "ktp", icon: "🪪", title: "KTP", desc: "Perbaikan data Kartu Tanda Penduduk", color: "#6b1a4b" },
];
...
export default function SistemPengaduan() {
  ...
  const handleRegister = async () => {
    try {
      ...
      alert("Register gagal, email mungkin sudah terdaftar!");
    }
  };
  ...
  const handleSubmit = async () => {
    try {
      ...
      setNomorPengaduan(res.data.nomor_pengaduan);
      setPage("sukses");
    } catch (err) {
      ...
      <header style={styles.header}>
        <div style={styles.headerInner}>
          ...
          <nav style={styles.nav}>
            ...
            {user ? (
              <button style={styles.navBtn} onClick={handleLogout}>Logout ({user.nama})</button>
            ) : (
              <button style={styles.navActive} onClick={() => setPage("auth")}>Login</button>
            )}
          </nav>
        </div>
      </header>
            ...
            <section style={styles.hero}>
              <div style={styles.heroContent}>
                ...
                <div style={styles.heroBtns}>
                  <button style={styles.btnPrimary} onClick={() => user ? (setPage("form"), setStep(0), setSelected(null)) : setPage("auth")}>
                    Buat Pengaduan Sekarang →
                  </button>
                  <button style={styles.btnOutline} onClick={() => setPage("status")}>Cek Status Pengaduan</button>
                </div>
              </div>
              ...
              <div style={styles.cardGrid}>
                {jenisPengaduan.map((j) => (
                  <div key={j.id} style={{ ...styles.layananCard, borderTop: `4px solid ${j.color}` }}
                    onClick={() => { user ? (setSelected(j.id), setPage("form"), setStep(1)) : setPage("auth") }}>
        ...
        {page === "auth" && (
          <div style={styles.formPage}>
            <div style={styles.formContainer}>
              <h2 style={styles.formTitle}>{authMode === "login" ? "Login" : "Register"}</h2>
              {authMode === "register" && (
                <div style={styles.formGroup}>
                  <label style={styles.label}>Nama Lengkap</label>
                  <input style={styles.input} placeholder="Nama lengkap"
                    value={authForm.nama} onChange={e => setAuthForm({ ...authForm, nama: e.target.value })} />
                </div>
              )}
              <div style={styles.formGroup}>
                <label style={styles.label}>Email</label>
                <input style={styles.input} placeholder="email@gmail.com"
              ...
              <div style={{ ...styles.formNav, marginTop: 24 }}>
              ...
              {step === 0 && (
                <div>
                  ...
                  <div style={styles.formNav}>
                    <div />
                    <button style={selected ? styles.btnPrimary : styles.btnDisabled} disabled={!selected} onClick={() => setStep(1)}>Lanjut →</button>
                  </div>
                </div>
              )}
                ...
                <div>
                  ...
                  <div style={styles.formNav}>
                    ...
                    <button style={form.nama && form.nik && form.telp ? styles.btnPrimary : styles.btnDisabled}
                      disabled={!form.nama || !form.nik || !form.telp} onClick={() => setStep(2)}>Lanjut →</button>
                  </div>
                </div>
              )}
                ...
                <div>
                  ...
                  <div style={styles.formNav}>
                    ...
                    <button style={form.deskripsi.length > 10 ? styles.btnPrimary : styles.btnDisabled}
                      disabled={form.deskripsi.length <= 10} onClick={() => setStep(3)}>Lanjut →</button>
                  </div>
                </div>
              )}
              ...
              {step === 4 && (
                <div>
                  ...
                  <div style={styles.formNav}>
                    ...
                    <button style={styles.btnSubmit} onClick={handleSubmit}>📨 Kirim Pengaduan</button>
                  </div>
                </div>
              )}
            </div>
          ...
          <div style={styles.formPage}>
            <div style={styles.formContainer}>
              ...
              <div style={styles.searchBox}>
                <input style={{ ...styles.input, flex: 1 }} placeholder="Masukkan nomor pengaduan" />
                <button style={styles.btnPrimary}>🔍 Cari</button>
              </div>
              <div style={{ marginTop: 32 }}>
                <div style={{ fontWeight: 700, color: "#475569", marginBottom: 16, fontSize: 14 }}>RIWAYAT PENGADUAN</div>
                {statusData.map((d) => (
        ...
        {page === "sukses" && (
          <div style={styles.formPage}>
            <div style={{ ...styles.formContainer, textAlign: "center" }}>
              ...
              <p style={styles.stepDesc}>Pengaduan Anda telah kami terima dan akan segera diproses.</p>
...
const styles = {
  ...
  navBtn: { padding: "8px 16px", border: "none", background: "transparent", cursor: "pointer", color: "#64748b", fontSize: 14, fontFamily: "inherit", borderRadius: 8 },
  ...
  layananCard: { background: "#fff", borderRadius: 16, padding: 28, cursor: "pointer", boxShadow: "0 2px 8px rgba(0,0,0,0.06)", border: "1px solid #e2e8f0" },
  ...
  selectCard: { borderRadius: 14, padding: 24, cursor: "pointer", textAlign: "center" },
  formGrid: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 },
  formGroup: { display: "flex", flexDirection: "column", gap: 6 },
  label: { fontWeight: 600, fontSize: 14, color: "#374151" },
  input: { padding: "10px 14px", border: "1.5px solid #e2e8f0", borderRadius: 10, fontSize: 15, fontFamily: "inherit", outline: "none" },
  ...
  uploadBox: { border: "2px dashed #cbd5e1", borderRadius: 16, padding: "48px 24px", textAlign: "center", cursor: "pointer", background: "#f8fafc" },
